...
class _QRPageState extends State<QRPage> {
  ...
  Future<void> convertQrCodeToImage(BuildContext context, String data, String name) async {
    ...
    const double textSize = 40.0;  // Adjust size based on the text height you expect
    ...
    final double imageHeight = qrImageHeight + 3 * margin + logoSize + textSize; // Adjusted height

    // Create a new image with the margins
    ui.PictureRecorder recorder = ui.PictureRecorder();
    Canvas canvas = Canvas(recorder, Rect.fromLTWH(0, 0, imageWidth, imageHeight));

    // Draw the QR code with margins
    canvas.drawRect(Rect.fromLTWH(0, 0, imageWidth, imageHeight), Paint()..color = Colors.white);
    canvas.drawImageRect(
      qrImage,
      Rect.fromLTWH(0, 0, qrImageWidth, qrImageHeight),
      Rect.fromLTWH(margin, margin + logoSize + textSize + margin, qrImageWidth, qrImageHeight),
      Paint(),
    );
    ...
    final textOffset = Offset(
      ...
      logoRect.bottom + 5, // 5 is the padding between the logo and the text
    );

    textPainter.paint(canvas, textOffset);

    // End recording and create the image
    ui.Image finalImage = await recorder.endRecording().toImage(imageWidth.toInt(), imageHeight.toInt());
    ...
    // Share the image
    await Share.shareFiles([imgFile.path], text: "Enjoy the ease of controlling your switches effortless operation, happy living!");
  }
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                ...
                child: Text("Share",style: TextStyle(color: appBarColour),),
              ),
              const SizedBox(
                height: 20,
              ),
