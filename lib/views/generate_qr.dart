              ...
              children: [
                ...
                DropdownMenu(
                    ...
                    dropdownMenuEntries: const [
                      DropdownMenuEntry(value: GenerateType.Switches, label: "Switches"),
                      DropdownMenuEntry(value: GenerateType.Routers, label: "Routers"),
                      DropdownMenuEntry(value: GenerateType.Groups, label: "Groups"),
                    ]
                ),
                const SizedBox(height: 10),
                if (generateType == GenerateType.Switches) const Text("Select Switch"),
                if (generateType == GenerateType.Routers) const Text("Select Router"),
                if (generateType == GenerateType.Groups) const Text("Select Group"),
                if (generateType == GenerateType.Switches)
                  DropdownMenu(
                    ...
                    dropdownMenuEntries: switches
                        .map((e) => DropdownMenuEntry(value: e.switchSSID, label: e.switchSSID))
                        .toList(),
                  ),
                if (generateType == GenerateType.Routers)
                  DropdownMenu(
                    ...
                    dropdownMenuEntries: routers
                        .map((e) => DropdownMenuEntry(value: e.name + "_" + e.switchName, label: e.name + "_" + e.switchName))
                        .toList(),
                  ),
                if (generateType == GenerateType.Groups)
                  DropdownMenu(
                    ...
                    dropdownMenuEntries: groups
                        .map((e) => DropdownMenuEntry(value: e.groupName, label: e.groupName))
                        .toList(),
                  ),
                const SizedBox(height: 10),
                ...
                Row(
                  ...
                  children: [
                    ElevatedButton(
                      onPressed: () {
                        if (generateType == GenerateType.Switches && switchh.switchld.contains("default")) {
                          showToast(context, "No switch is selected");
                          return;
                        }

                        if (generateType == GenerateType.Routers && router.switchName.contains("default")) {
                          showToast(context, "No router is selected");
                          return;
                        }
                        if (generateType == GenerateType.Groups && group.groupName.contains("default")) {
